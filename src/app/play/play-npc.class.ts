  ...
  npcLogic(): void {
    if (this.npcData !== null) {
      if (this.npcData.isNPC && !this.character!.fixedPosition && !this.dying) {
        ...
        const distance: number = this.scenario!.player!.blockPos.distance(this.blockPos);
        if (distance > 250) {
          this.npcData.status = 'wandering';
        }

        if (this.npcData.status === 'wandering' && this.npcData.remainingTime < 1) {
          const movementOptions: Orientation[] = ['down', 'up', 'left', 'right'];
          const currentInd: number = movementOptions.findIndex(
            (x: Orientation): boolean => x === this.orientation,
          );
          movementOptions.splice(currentInd, 1);
          const option: Orientation =
